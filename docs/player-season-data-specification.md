...
7. **Recent Form V1 (Implementation 2) IMPLEMENTED — §5.3–§5.8 SUPERSEDED** (Phase D doc-only delta marker, checkpoint `pitchlog-recent-form-v1-phase-d`): the draft's recent-form / trends / with-without-player / consistency text (**§5.3–§5.8**) is superseded, wherever the two differ, by **`docs/recent-form-trends-specification.md`** — Document ID **`PitchLog-RECENT-FORM-SPEC-v1.0-reestablished`** — per that document §0.2 and §34 PHASE D. The draft text remains in place as historical review text; no methodology text was silently rewritten. This item supersedes item 5's "still not implemented" statement above (recorded, not rewritten, so the delta history stays auditable). Implementation checkpoints: **`8aee44e`** (`pitchlog-recent-form-v1-reestablished-core` — pure engine `src/recent-form.js` + `tests/recent-form-tests.js`, 245 checks), **`374d183`** (`pitchlog-recent-form-v1-ui-integration` — read-only season-view UI integration + `tests/recent-form-ui-check.js`, 124 checks), **`5ebb480`** (`pitchlog-recent-form-v1-specification-doc` — the governing Recent Form specification committed). All pre-existing suites pass **unmodified** at those checkpoints (baseline 944 checks + matchday-sim PASS, per RF spec §34/§36).
...
One row per player×match + season-summary rows per player. Columns (exact, ordered): `match_key, date, opponent, competition, home_away, result, x1_status, player_id, player_name, player_number, participation_status, started, subbed_on, subbed_on_min, subbed_off, subbed_off_min, sent_off, minutes_est, minutes_quality, minutes_reasons, goals, shots, shots_on_target, chances, key_passes, crosses, passes, pass_success_pct, presses, press_wins, interceptions, recoveries, turnovers, duels, fouls, yellow_cards, red_cards, transitions_positive, transitions_negative, positive_events, negative_events, events_total, located_events, unlocated_events, located_share_pct, events_1h, events_2h, events_et1, events_et2, state_winning, state_drawing, state_losing, state_suppressed, zone_dl, zone_dc, zone_dr, zone_ml, zone_mc, zone_mr, zone_al, zone_ac, zone_ar, zone_unlocated`. BOM/CRLF conventions and `csvEscape` reuse the existing exporters verbatim (since R2-A the shared exporter write path prepends a UTF-8 BOM at file-write time; the engine's CSV string itself remains BOM-free). **`team`-era semantics** (the legacy export's `side` column is not replicated).

**SEASON_SUMMARY rows (documented R2-A; the 63-column contract is unchanged).** Each player's summary row is explicitly distinguished — no extra column needed — by two sentinel cells: `match_key = 'SEASON_SUMMARY'` (column 1) and `participation_status = 'SEASON_SUMMARY'` (column 11). On summary rows the per-match context columns (date, opponent, competition, home_away, result, x1_status) and the per-match participation columns (started, subbed_on, subbed_on_min, subbed_off, subbed_off_min, sent_off) are empty; `minutes_est` is the season estimated-minutes rollup with the season minutes-quality code and reason rollup; all count/percentage/spatial/period/state cells come from the season pooled rollups (never averages of per-match percentages). Row order: player-major — a player's match rows in season order, then that player's SEASON_SUMMARY row. Adding a dedicated `row_type` column would break the pinned 63-column contract, so the two-sentinel labeling above IS the explicit row distinction; consumers filter on `match_key = 'SEASON_SUMMARY'`.
